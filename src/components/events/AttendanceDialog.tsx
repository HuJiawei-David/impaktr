...
export function AttendanceDialog({
  ...
  return (
    <div 
      ...
      onWheel={(e) => {
        // Only prevent scroll if we're scrolling the backdrop, not the dialog content
        const target = e.target as HTMLElement;
        if (target === e.currentTarget || target.closest('.dialog-backdrop')) {
          e.preventDefault();
          e.stopPropagation();
        }
      }}
    ...
    >
      {/* Backdrop */}
      <div 
        className="dialog-backdrop fixed inset-0 bg-black/50 backdrop-blur-sm"
        onClick={handleClose}
      />
      
      {/* Dialog */}
      <div 
        className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4 my-auto max-h-[90vh] flex flex-col animate-in fade-in-0 zoom-in-95 duration-200"
        onClick={(e) => {
      ...
      >
        ...
        <div className="absolute right-4 top-4 z-50">
          <button
            ...
            className="rounded-sm opacity-70 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none bg-white dark:bg-gray-800 p-1 shadow-sm"
          >
